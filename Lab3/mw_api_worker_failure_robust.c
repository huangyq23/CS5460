...
#include <stdlib.h>
#include <stdio.h>
#include <mw_api_worker_failure_robust.h>
#include <mpi.h>
#include <string.h>
...
#define true 1
#define false 0
static void master(int argc, char **argv, mw_api_spec *f, time_t timeout);
static void worker(int argc, char **argv, mw_api_spec *f, float fp);
...
void MW_Run (int argc, char **argv, mw_api_spec *f, float fp, time_t timeout)
{
    ...
    if (myid == 0)
        master(argc, argv, f, timeout);
    else
        worker(argc, argv, f,fp);
    MPI_Finalize();
}
...
//pick a live worker to the master's best knowledge
static int pick_dest(bool* failed, int size){
    int dest = 1;
    while(dest<size){
      if(failed[dest])    
...
static void master(int argc, char **argv, mw_api_spec *f, time_t timeout)
{
    ...
    {
        MPI_Send((char*)work + work_to_send*(f->work_sz),
                 f->work_sz,
                 MPI_BYTE,
                 rank,
    ...
    while (work_finished < work_count)
    {
       ...
       if(!flag){
           ...
           if(fail==size-1){// whether all workers are failed?
               printf("%s",terminate_msg);
               MPI_Finalize();
               exit(1);
           }
           for(int i=0;i<work_count;i++){
               if(!get_result[i]){//if a work is unfinished(master haven't got the result), pick a new worker.
                  ...
                  work_worker[i] = dest;
                  resend++;
                  printf("send work %d to %d\n",i,dest);
                 // break;
               }
     	   }
       }
    }
    //Terminate all workers, nonblocking send,those who are alive will get it!
    for (rank = 1; rank < size; rank++)
    {
        MPI_Isend(0,
                 0,
...
static void worker(int argc, char **argv, mw_api_spec *f, float failure_p)
{
    MPI_Status status;
